...
// 1. Добавляем декларации переменных после "const accessSection = document.getElementById('access-section');"
...
const targetVar = "const accessSection = document.getElementById('access-section');";
if (content.includes(targetVar) && !content.includes('const addAddressModal')) {
  ...
  console.log('Added variable declarations');
}
...
function populateModalZoneSelect() {
  ...
  modalZoneSelect.innerHTML = '<option value="">— Зона —</option>';
...
if (!content.includes('function populateModalZoneSelect()')) {
  content = content.replace(targetFunc, populateModalFunc + '\n\n' + targetFunc);
  console.log('Added populateModalZoneSelect function');
}
...
if (!content.includes('// --- Modal voice input')) {
  ...
  console.log('Added modal and mic code');
}
